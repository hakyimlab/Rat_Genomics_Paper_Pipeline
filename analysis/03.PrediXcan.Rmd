...
data.dir <- "/gpfs/data/im-lab/nas40t2/natasha/rat_genomics/rat-genomic-analysis/"
filename <-  data.dir %&% "Results/sql/Ac_output_db.db"
...
model_db = MODEL %&% "Ac_prediXcan_db.db"
...
--model_db_path $MODEL/Ac_prediXcan_db.db \
--text_genotypes  \
...
```{r }
pred_expr <- read_tsv("/gpfs/data/im-lab/nas40t2/natasha/rat_genomics/prediXcan/metabolic_traits/rat_metabolic_" %&% args[1] %&% "__predict.txt", col_names = TRUE)
...
tempo_df <- full_df %>% filter(pvalue < 5.767678e-06)
...
full_df <- full_df %>% mutate(bf_sig = ifelse(full_df$pvalue <= 5.767678e-06, "Yes", "No"))
```



```{r find genes that overlap in Humans, eval=FALSE}
human_height_genes <- read_tsv("/Users/natashasanthanam/Downloads/Human_prediXcan_all_traits.txt", col_names = TRUE)
human_height_genes <- human_height_genes %>% mutate(rat_gene = orth.rats[match(human_height_genes$gene_name, orth.rats$external_gene_name), 4]$rnorvegicus_homolog_associated_gene_name) %>% filter(abs(zscore_Height) >= 3.719)


human_bmi_genes <- read_tsv("/Users/natashasanthanam/Downloads/Human_prediXcan_all_traits.txt", col_names = TRUE) 
colnames(human_bmi_genes)[3] = "zscore_BMI"
human_bmi_genes <- human_bmi_genes %>% mutate(rat_gene = orth.rats[match(human_bmi_genes$gene_name, orth.rats$external_gene_name), 4]$rnorvegicus_homolog_associated_gene_name) %>% filter(abs(zscore_BMI) >= 3.719 )
```
...
bmi_manhat <- tempo_manhatt %>% filter(metabolic_trait == "bmi_bodylength_w_tail") 
...
height_manhat <- tempo_manhatt %>% filter(metabolic_trait == "bodylength_w_tail") 
...
bmi_manhplot <- ggplot(gwas_data, aes(x = bp_cum, y = -log10(pvalue), 
  ...
  geom_label_repel(aes(label=ifelse(pvalue <=  sig, gene_name, "")), size = 6) + 
  ylim(c(0,8)) + 
...
FUMA analysis
```{r FUMA analysis, eval=TRUE}
gene_annot_hum <- fread("/Users/natashasanthanam/GTEx-Analysis/annotations_gencode_v26.tsv")
height_annot <- gene_annot_hum[na.omit(match(height_df$gene_name, gene_annot_hum$gene_name)),]
height_annot <- height_annot %>% select(c(gene_name, chromosome, start))


height_df <- full_df %>% filter(metabolic_trait == "bodylength_w_tail") %>% filter(pvalue <= 5.767678e-06) %>% select(c(gene_name)) %>% na.omit()
height_df <- inner_join(height_df, height_annot, by = "gene_name") 
height_df <- height_df %>% mutate(SNP = paste(height_df$chromosome, height_df$start, sep= ":")) %>% select(c(gene_name, SNP))


bmi_annot <- gene_annot_hum[na.omit(match(bmi_df$gene_name, gene_annot_hum$gene_name)),]
bmi_annot <- bmi_annot %>% select(c(gene_name, chromosome, start))
bmi_df <-  full_df %>% filter(metabolic_trait == "bmi_bodylength_w_tail") %>% filter(pvalue <= 5.767678e-06) %>% select(c(gene_name)) %>% na.omit()

bmi_df <- inner_join(bmi_df, bmi_annot, by = "gene_name") 
bmi_df <- bmi_df %>% mutate(SNP = paste(bmi_df$chromosome, bmi_df$start, sep= ":")) %>% select(c(gene_name, SNP))
```


Loci Analysis
```{r function to count distinct genes}
fn_count_distinct_loci = function(df_loci) {
all_distinct_loci <- tibble(chr= as.numeric(), gene_id = as.character())
for(i in 1:20) {
  if(i %in% df_loci$chr) {
    tempo <- df_loci %>% filter(chr == i)
    tempo <- df_loci[order(df_loci$start),]
    for(j in 1:(nrow(tempo)-1)) {
      if(nrow(tempo) == 1) {
        all_distinct_loci <- full_join(tempo %>% select(c(chr, gene_id)), all_distinct_loci, by = c("chr", "gene_id"))
      }
      else if(abs(tempo$start[j+1] - tempo$start[j]) >= 1e+06) {
           all_distinct_loci <- full_join(tempo[j,1:2], all_distinct_loci, by = c("chr", "gene_id"))
        }
        else { 
          j = j+1
        }
...
height_loci <- full_df %>% filter(metabolic_trait == "bodylength_w_tail") %>% filter(pvalue <= 5.767678e-06)

height_loci <- inner_join(gene_annot, height_loci %>% select(gene) %>% rename(gene_id = gene), by = "gene_id")
...
bmi_loci <- full_df %>% filter(metabolic_trait == "bmi_bodylength_w_tail") %>% filter(pvalue <= 5.767678e-06)

bmi_loci <- inner_join(gene_annot, bmi_loci %>% select(gene) %>% rename(gene_id = gene), by = "gene_id")
