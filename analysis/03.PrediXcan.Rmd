...
filename <-  data.dir %&% "models/EN_models/" %&% tis %&% "_output_db.db"
...
cor_df <- read_tsv(data.dir %&% "rat-genomic-analysis/Results/all_results_" %&% tis, col_names = TRUE) %>% select(c(gene, cor))

extra <- extra %>% filter(pred.perf.R2 > 0.01)

extra <- full_join(extra, cor_df, by = "gene")
extra <- extra %>% filter(cor >= 0 | is.na(cor)) %>% select(-c(cor)) %>% filter(!is.na(pred.perf.R2))
weights <- weights %>% filter(gene %in% extra$gene)
```


```{r create database connection, eval=FALSE}
model_db = MODEL %&% tis %&% "_best_prediXcan_db.db"
...
--prediction_output rat_metabolic_Ac_best__predict.txt  \
--prediction_summary_output rat_metabolic_Ac_best__summary.txt \
--throw
```
...
Distribution of Effect Sizes from PrediXcan
```{r}
hist(full_df$effect)
summary(full_df$effect)
```
```{r}
filename <- "/Users/natashasanthanam/Box/imlab-data/data-Github/rat-genomic-analysis/sql/Tyson_original_models/Ac_output_db.db"
 sqlite.driver <- dbDriver("SQLite")
conn <- dbConnect(RSQLite::SQLite(), filename)
weights <- dbGetQuery(conn, 'select * from weights')
hist(weights$weight)
dbDisconnect(conn)
```
plot predicted vs observed expression for Eif3a
```{r}
Eif3a <- read_tsv("/Users/natashasanthanam/Downloads/model_perf_Eif3a.txt", col_names = TRUE)
ggplot(Eif3a, aes(pred, obs)) + geom_point()

```
Remove genes with bad effect size
```{r}
full_df %>% mutate(R2 = extra[match(full_df$gene, extra$gene),3]) %>% filter(abs(effect) > 30) %>% select(c(R2)) %>% summary()
