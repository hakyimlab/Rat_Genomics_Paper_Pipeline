...
gene_annot <- readRDS("/Users/natashasanthanam/Github/rat-genomic-analysis/data/gene_annotation.RDS") %>% select(c("chr", "gene_id", "gene_name", "start", "end")) %>% rename(gene = gene_id)

tempo_manhatt <- inner_join(gene_annot, full_df, by = "gene")
tempo_manhatt$chr <- as.numeric(tempo_manhatt$chr)

bmi_manhat <- tempo_manhatt %>% filter(metabolic_trait == "bmi_bodylength_w_tail" | metabolic_trait == "bmi_bodylength_wo_tail")

height_manhat <- tempo_manhatt %>% filter(metabolic_trait == "bodylength_w_tail" | metabolic_trait == "bodylength_wo_tail" | metabolic_trait == "tail_length")

manhattan(bmi_manhat, chr="chr", bp="start", snp="gene_name", p="pvalue", ylim = c(0, 13), suggestiveline = T, genomewideline = T, annotatePval = 5.767678e-07, main = "Manhattan plot of Significant BMI Associations")
abline(h= 6.238999, col="blue")

manhattan(height_manhat, chr="chr", bp="start", snp="gene_name", p="pvalue",  suggestiveline = T, genomewideline = T, annotatePval = 5.767678e-07, main = "Manhattan plot of Significant Body Length Associations")
abline(h= 6.238999, col="blue")
