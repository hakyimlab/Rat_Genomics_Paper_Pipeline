...
For each trait find the mean zscore across all tissues
...
for(i in 2:ncol(pheno)) {
  ...
  for(fila in filelist) {
    df <- read_tsv(fila) %>% select(c(gene, zscore))
   tempo <- full_join(tempo, df,  by = "gene")
  }
  tempo <- tempo %>% mutate (zscore.mean = rowMeans(tempo[,2:ncol(tempo)], na.rm = TRUE)) %>% select(c(gene, zscore.mean))
  write_tsv(tempo, data.dir %&% "prediXcan/metabolic_traits/associations/average_zscores/" %&% trait %&% "_avg_zscore.txt", col_names = FALSE)
}
```
