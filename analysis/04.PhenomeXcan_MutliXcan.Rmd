...
z_list <- list.files("/gpfs/data/im-lab/nas40t2/natasha/rat_genomics/prediXcan/metabolic_traits/associations/average_zscores", pattern = "zscore", full.names = TRUE)
...
for(fila in filelist) {
  ...
  z_df <- read_tsv("/gpfs/data/im-lab/nas40t2/natasha/rat_genomics/prediXcan/metabolic_traits/associations/average_zscores/" %&% trait %&% "_avg_zscore.txt", col_names = FALSE) %>% rename(gene = X1) %>% rename(zscore.mean = X2)
  
  z_df <- z_df[order(match(z_df$gene, tempo$gene)), ]
  tempo <- tempo %>% mutate(zscore = abs(qnorm(pvalue / 2) ) * sign(z_df$zscore.mean))
...
```{r pllot zscores from MultiXcan}
ggplot(all_genes, aes(zscore.fasting_glucose, Fasting_Glucose)) + geom_point()  + xlab("Fasting Glucose in rats") + ylab("Fasting Glucose in Humans")  + geom_vline(xintercept = 0, color = "grey") +  geom_hline(yintercept = 0, color = "grey") + geom_label_repel(aes(label=ifelse((zscore.fasting_glucose > 1.96 & Fasting_Glucose > 1.96)| (zscore.fasting_glucose < -1.96 & Fasting_Glucose < -1.96), as.character(gene_name),'')), force = 0.05, size = 3, box.padding   = 0.1, point.padding = 0.2)

ggplot(all_genes, aes(zscore.bodylength_w_tail, Height)) + geom_point()  + xlab("Bodylength with tail in rats") + ylab("Height in Humans")  + geom_vline(xintercept = 0, color = "grey") +  geom_hline(yintercept = 0, color = "grey") + geom_label_repel(aes(label=ifelse((zscore.bodylength_w_tail > 1.96 & Height > 1.96)| (zscore.bodylength_w_tail < -1.96 & Height < -1.96), as.character(gene_name),'')), force = 0.05, size = 3, box.padding   = 0.1, point.padding = 0.2)

ggplot(all_genes, aes(zscore.bodyweight, Body_Fat)) + geom_point()  + xlab("Bodyweight in rats") + ylab("Body Fat in Humans")  + geom_vline(xintercept = 0, color = "grey") +  geom_hline(yintercept = 0, color = "grey") + geom_label_repel(aes(label=ifelse((zscore.bodyweight > 1.96 & Body_Fat > 1.96)| (zscore.bodyweight < -1.96 & Body_Fat < -1.96), as.character(gene_name),'')), force = 0.05, size = 3, box.padding   = 0.1, point.padding = 0.2)

ggplot(all_genes, aes(zscore.bmi_bodylength_w_tail, BMI)) + geom_point()  + xlab("BMI with Tail in rats") + ylab("BMI in Humans")  + geom_vline(xintercept = 0, color = "grey") +  geom_hline(yintercept = 0, color = "grey") + geom_label_repel(aes(label=ifelse((zscore.bmi_bodylength_w_tail > 1.96 & BMI > 1.96)| (zscore.bmi_bodylength_w_tail < -1.96 & BMI < -1.96), as.character(gene_name),'')), force = 0.05, size = 3, box.padding   = 0.1, point.padding = 0.2)
```
