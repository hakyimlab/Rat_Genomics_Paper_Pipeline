...
for(fila in filelist) {
  ...
  df <- z_df[na.omit(match(tempo$gene, z_df$gene)), ]
  
  tempo <- tempo %>% mutate(zscore = abs(qnorm(pvalue / 2) ) * sign(df$zscore.mean))
...
ggplot(all_genes, aes(zscore.bmi_bodylength_w_tail, BMI)) + geom_point()  + xlab("BMI in rats including Tail") + ylab("BMI in Humans") + ggtitle("Compare MultiXcan Zscores between Rats and Humans")
cor.test(all_genes$zscore.bmi_bodylength_w_tail, all_genes$BMI, method = "spearman")
```
...
ggplot(all_genes, aes(zscore.bodyweight, Body_Fat)) + geom_point()  + xlab("Bodyweight in rats") + ylab("Body Fat in Humans")  + geom_vline(xintercept = 1.30103) +  geom_hline(yintercept = 1.30103) + geom_label_repel(aes(label=ifelse((zscore.bodyweight >= 1.645 | Body_Fat >= 1.645), as.character(gene_name),''), box.padding   = 0.35, point.padding = 0.5)) 
...
ggplot(all_genes, aes(zscore.epifat, Body_Fat)) + geom_point()  + xlab("Epifat in rats") + ylab("Body Fat in Humans")  + geom_vline(xintercept = 1.645) +  geom_hline(yintercept = 1.645) + geom_label_repel(aes(label=ifelse((zscore.epifat >= 1.645 | Body_Fat >= 1.645), as.character(gene_name),''), box.padding   = 0.35, point.padding = 0.5))
...
ggplot(all_genes, aes(zscore.fasting_glucose, Fasting_Glucose)) + geom_point()  + xlab("Fasting glucose in rats") + ylab("Fasting Glucose  in Humans")  + geom_vline(xintercept = 1.645) +  geom_hline(yintercept = 1.645) + geom_text_repel(aes(label=ifelse((zscore.fasting_glucose >= 1.645 | zscore.fasting_glucose <= -1.645), as.character(gene_name),''), box.padding   = 0.1, point.padding = 0.1, max.overlaps = 40))
