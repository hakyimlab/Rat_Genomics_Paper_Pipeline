...
ggplot(all_genes, aes(zscore.bodylength_w_tail, BMI)) + geom_point()  + xlab("Bodylength with tail in rats") + ylab("BMI in Humans")  + geom_vline(xintercept = 0, color = "grey") +  geom_hline(yintercept = 0, color = "grey") + geom_label_repel(aes(label=ifelse((zscore.bodylength_w_tail > 1.96 & BMI > 1.96)| (zscore.bodylength_w_tail < -1.96 & BMI < -1.96), as.character(gene_name),'')), force = 0.05, size = 2)
...
ggplot(all_genes, aes(zscore.bodyweight, Body_Fat)) + geom_smooth()
...
fasting_glucose_rat_genes <- pheno_rats %>% filter(pvalue.fasting_glucose <= 0.05) %>% .[["gene_name"]] %>% na.omit()
