...
epifat_genes <- pheno_rats %>% filter(pvalue.epifat <= 1e-4) %>% .[["gene_name"]] %>% na.omit()
...
height_genes_rats <- pheno_rats %>% filter(pvalue.bodylength_w_tail <= 1e-4) %>% .[["gene_name"]] %>% na.omit()
...
height_genes_rats <- pheno_rats %>% filter(pvalue.bodylength_wo_tail <= 1e-4) %>% .[["gene_name"]] %>% na.omit()
qqpoints(height_genes %>% filter(gene_name %in% height_genes_rats) %>% .[["pvalue"]],pch='+', col=3)
