...
fila <- "/Users/natashasanthanam/Box/imlab-data/data-Github/rat-genomic-analysis/sql/Lh_output_db.db"
sqlite.driver <- dbDriver("SQLite")
conn <- dbConnect(RSQLite::SQLite(), fila)
...
cor.test(all_r2$R2, all_r2$en_r2, method = "spearman")
```

Plot the Comparison between the two 
```{r save inner join of r2 and h2}
Lh_h2 <- read_tsv("/Users/natashasanthanam/Github/rat-genomic-analysis/data/Lh_PVE_estimates.txt", col_names = FALSE) %>% dplyr::rename(rat_h2 = X2) %>% dplyr::rename(gene.x = X1) %>% mutate(rat_ci = paste(round(Lh_h2$X3, digits = 5), round(Lh_h2$X4, digits = 5), sep = "-")) %>% dplyr::select(c(gene.x, rat_h2, rat_ci))

lg_df <- inner_join(all_r2, Lh_h2, by = "gene.x")

lg_df <- lg_df %>% dplyr::select(c(ensid, gene.x, genename, R2, en_r2,rat_h2,h2,  rat_ci, h2_ci))
...
lg_df %>% mutate(human_shuffled_h2 = sample(human_h2, nrow(lg_df), replace=F)) %>% ggplot(aes(rat_h2, human_h2)) + geom_smooth() + geom_smooth(aes(rat_h2, human_shuffled_h2),col="dark gray")+theme_bw()+ xlab("Rat Heritability") + ylab("Human Heritability")

# Plot R2 between rats and Humans with line for null 
lg_df %>% mutate(human_shuffled_r2 = sample(human_r2, nrow(lg_df), replace=F)) %>% ggplot(aes(rat_r2, human_r2)) + geom_smooth() + geom_smooth(aes(rat_r2, human_shuffled_r2),col="dark gray")+theme_bw()+ xlab("Rat Predictability") + ylab("Human Predictability")
```
