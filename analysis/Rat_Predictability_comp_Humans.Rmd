...
AC0_pred <- as.data.frame(pred_expr[, c("FID", "ENSG00000236682.1")])
AC0_obs <- as.data.frame(obs_expr[,  c("FID", "ENSG00000236682.1")]) 

AC0_pred <- AC0_pred[match(AC0_obs$FID, AC0_pred$FID),]
AC0_obs <- AC0_obs[match(AC0_pred$FID, AC0_obs$FID ), ]


AC0 <- inner_join(AC0_pred, AC0_obs, by = "FID")
colnames(AC0) = c("FID", "pred", "obs")

AC0_GTEx <- read_tsv("/Users/natashasanthanam/Downloads/GTEx_AC0_results.txt")
g4 <- ggplot(AC0_GTEx, aes(pred, obs)) + geom_point() + annotate("text", x = -0.4, y = 1.1, label = "AC068282.3", size = 10, fontface =2) + annotate("text", x = -0.55, y = 1, label = "r2 =  0.727", size = 8) + theme(aspect.ratio=1) + xlab("Predicted expression") + ylab("Observed Expression")
```
...
filelist <- c("en_Brain_Amygdala.db", "en_Brain_Cerebellum.db", "en_Brain_Nucleus_accumbens_basal_ganglia.db", "en_Brain_Substantia_nigra.db")

pred_tis <- data.frame(gene = as.character())
for(fila in filelist) {
  filename <- model.dir %&% fila
  tis <- substr(fila, 4, str_length(fila)-3)
  sqlite.driver <- dbDriver("SQLite")
  conn <- dbConnect(RSQLite::SQLite(), filename)
  extra <- dbGetQuery(conn, 'select * from extra')
  extra <- extra %>% select(c(gene, pred.perf.R2)) 
  colnames(extra)[2] = tis
  pred_tis <- full_join(pred_tis, extra, by = "gene") 
  dbDisconnect(conn)
}
...
rownames(pred_tis) <- pred_tis$gene
pred_tis <- pred_tis %>% select(-c(gene))
colnames(pred_tis) = c("Amygdala", "Cerebellum", "Nucleus accumbens", "Substantia nigra")
