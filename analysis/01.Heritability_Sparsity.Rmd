...
plt_1 <- (ggplot(data = A_df_Ac, aes(x = index))
          + geom_point(aes(x=index, y=pred.perf.R2), colour = "dodgerblue2", size = 0.2)
          ...
          + labs(x = 'Genes Sorted by Proportion of Variance Explained (PVE)',
                 y = 'PVE')
           + ylim(-0.5,1)
           + annotate("text", x = 1250, y = 0.9, label = "Mean h2 =  0.089", size = 3.5)
           + annotate("text", x = 1250, y = 0.8, label = "Mean r2 =  0.085", size = 3.5)) 
plt_1
```
...
```{r plot human h2}
human_h2 <- read_csv("/Users/natashasanthanam/Github/rat-genomic-analysis/data/human_h2.csv") %>% rename(point_estimate = h2) 

human_h2$credible_set_1 = as.numeric(ifelse(substr(human_h2$h2_ci ,1,1) == "-", paste("-", sapply(strsplit(human_h2$h2_ci, "-"), `[`, 2), sep = ""), sapply(strsplit(human_h2$h2_ci, "-"), `[`, 1)))
human_h2$credible_set_2 = as.numeric(ifelse(substr(human_h2$h2_ci ,1,1) == "-", sapply(strsplit(human_h2$h2_ci, "-"), `[`, 3), sapply(strsplit(human_h2$h2_ci, "-"), `[`, 2)))
...
plt_2  <- (ggplot(data = hum_df, aes(x = index))
          ...
          + labs(x = 'Genes Sorted by Proportion of Variance Explained (PVE)',
                 y = 'PVE')
           + ylim(-0.5,1)
           + annotate("text", x = 1250, y = 0.9, label = "Mean h2 =  0.126", size = 3.5)
           + annotate("text", x = 1250, y = 0.8, label = "Mean r2 =  0.119", size = 3.5)) 
plt_2
```
