...
```{r read in expression files, eval=TRUE}
...
```{r read in bim files for each tissue, eval=TRUE}
...
Repeat for Il:
```{r generate cis herit for Il, eval=TRUE}
...
The following code uses `bim_bam` files generated through Palmer lab data, steps [here](convert_genotypes_old.Rmd):
