...
tempo <- read_tsv(filelist[1]) %>% filter(R2 <= 1) %>% select(c(gene, cor)) %>% distinct(gene, .keep_all = TRUE)
...
for (k in 2:(length(filelist))){
 ...
 df <- as.data.frame(df) %>% select(c(gene, cor)) %>% filter(cor <= 1)
...
tempo <- tempo %>% filter(Average > 0.05)
...
data_long$value = as.numeric(substr(data_long$value, 5, str_length(data_long$value)))
