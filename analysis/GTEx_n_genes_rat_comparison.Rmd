...
tempo_R2_df$Average = rowMeans(subset(tempo_R2_df, select = -c(gene)))
tempo_R2_df <- tempo_R2_df %>% filter(Average >= 0.09)
...
```{r}
data_long <- tempo_R2_df %>% select(-c(Average)) %>%  pivot_longer(!gene, names_to = "value", values_to = "count") 
